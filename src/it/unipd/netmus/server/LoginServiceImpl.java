...
public class LoginServiceImpl extends RemoteServiceServlet implements
     ...
     * LoginHelper e ritorna, se presenti, le informazioni di base
     * (UserSummaryDTO) dell’utente loggato.
     */
    @Override
    ...
    /**
     * Inserisce i dati di login relativi alla registrazione di un nuovo utente
     * nel database. Può lanciare eccezioni di tipo <= RegistrationException in
     * caso vi siano problemi nell’inserimento.
     */
    @Override
    public LoginDTO insertRegistration(LoginDTO login)
    ...
    /**
     * Recupera un eventuale sessione salvata nei cookie controllandone la
     * validità per l’utente dato in input. Questa funzionalità non è valida per
     * gli utenti Google dei quali non viene gestito un cookie.
     */
    @Override
    public String restartSession(String user, String session_id)
        ...
        if (userLoggedIn != null)
            ...
            if (session_id_old.equals(session_id)) {
                ...
                // non c'e' sessione, ci sono cookie, ma son estranei (altra
                // macchina)
                throw new LoginException();
            }
        }
    ...
    /**
     * Racchiude tutte i passi di autenticazione dell’utente. Utilizza
     * verifyLogin e si occupa di iniziare la sessione dell’utente che si è
     * autenticato.
     */
    @Override
    ...
    /**
     * Metodo fondamentale per l’autenticazione che confronta i dati in input
     * con quelli presenti nel database per verificare se il login è valido. Può
     * lanciare eccezioni di tipo <= LoginException in caso il controllo non
     * vada a buon fine.
     */
