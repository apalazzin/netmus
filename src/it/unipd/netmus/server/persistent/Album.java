 ...
 * Cacheable poiché viene gli album vengono gestiti anche nella Memcache. Ogni
 * album è rappresentato da una chiave univoca (id) e dal link alla copertina ad
 * esso associata. Questa classe ha lo scopo di mantenere nel Datastore tutte le
 * copertine ricercate su Last.fm in modo ordinato, minimizzando così le
 * richieste esterne.
 */
...
public class Album implements Serializable, Cacheable {
     ...
     * dati in input. Se l'album non è presente nel Datastore o se la copertina
     * non è ancora stata ricercata su Last.fm viene ritornata la stringa vuota.
     */
    ...
    /**
     * Ritorna il link alla copertina dell'album associato al nome e all'artista
     * dati in input. Se l'album non è presente nel Datastore viene effettuata
     ...
     * Ritorna false nel caso in cui l'album specificato sia già presente.
     */
    ...
    /**
     * Chiave univoca assegnata ad un album, è generata a partire dal nome
    ...
    /**
     * Inserisce nel database per la prima volta l'album. Se esiste già un album
